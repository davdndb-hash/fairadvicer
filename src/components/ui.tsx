        ...
        <Reveal as="li" key={item.title} delay={i * 60}>
          ...
          <p
            className={`mt-2 text-[0.94rem] leading-relaxed pretty ${
              light ? "text-brand-100/80" : "text-ink-soft"
            }`}
          >
            {item.body}
          </p>
      ...
      {steps.map((s, i) => (
        <Reveal
          ...
          className={`group relative p-7 transition-colors lg:p-8 ${
            light ? "bg-brand-950 hover:bg-brand-900" : "bg-white hover:bg-brand-50/60"
          }`}
        >
          <span
            className={`font-display text-[2.6rem] leading-none ${
              light ? "text-white/20" : "text-brand-100"
          ...
          <p className={`mt-2 text-[0.92rem] leading-relaxed pretty ${light ? "text-brand-100/80" : "text-ink-soft"}`}>
            {s.body}
          </p>
        </Reveal>
      ))}
        ...
        <li key={item} className="flex items-start gap-3">
          <span
            className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${
              light ? "bg-accent-500/20 text-accent-400" : "bg-brand-100 text-brand-700"
            }`}
          >
            <Icon name="check" className="h-3 w-3" strokeWidth={2.4} />
          </span>
          <span className={`text-[0.95rem] leading-relaxed ${light ? "text-brand-100/85" : "text-ink-soft"}`}>
            {item}
          </span>
        </li>
      ))}
  ...
  return (
    <section className="relative overflow-hidden bg-brand-900 text-white">
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(209,94,0,.35) 0%, transparent 62%)" }}
      />
      <div className="container-x relative grid gap-12 py-20 lg:grid-cols-2 lg:gap-20 lg:py-28">
        <div>
          ...
          <div className="mt-9 grid gap-3 text-[0.95rem]">
            <a href="tel:+4989242076100" className="flex items-center gap-3 text-brand-100 transition-colors hover:text-white">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-accent-400">
                <Icon name="phone" className="h-4 w-4" />
              </span>
              +49 89 24 20 76 100
            </a>
            <a href="mailto:info@fairadvicer.de" className="flex items-center gap-3 text-brand-100 transition-colors hover:text-white">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-accent-400">
                <Icon name="mail" className="h-4 w-4" />
              </span>
              info@fairadvicer.de
            </a>
            <p className="flex items-center gap-3 text-brand-100/80">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-accent-400">
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[0.75rem] font-medium ${
        light ? "border-white/20 text-brand-100" : "border-line bg-white text-ink-soft"
      }`}
    >
      {children}
    </span>
  ...
  return (
    <section className="relative overflow-hidden border-b border-line bg-paper-2/60">
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(53,112,77,.16) 0%, transparent 65%)" }}
      />
